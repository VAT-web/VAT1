Feature: Aviva Signature Monthly Income Plan-LI=PRO-WITHOUTEkyc
  ...
  @PRODUCT
  Scenario: MIP- CONTINUATION- From Self/Reference home page
...
#    And Selected the Age of Life Insured or Policy Holder in Need Analysis tab
    ...
    And Down to Bottom
    And Select the product MIP
    And CLick on BI button
    And Click proceed in alert
#    And Select the Gender
    And Payment Frequency MIP
    And Plan Option MIP
    And Premium Payment Term MIP
    And Annual Premium MIP
    And Down to Bottom
    And Clicked on BI Generate Button MIP
    And Scroll to TOP
